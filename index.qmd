...
server <- function(input, output, session) {
  ...
  session$sendCustomMessage(
    ...
    output$main_table <- DT::renderDT({
        if (is.null(input$process_data) || input$process_data == 0) 
        
            make_scrolling_table(head(original_data(), 50))
            ...
            make_scrolling_table(df)
            
        } else make_scrolling_table(anonymised_data())
